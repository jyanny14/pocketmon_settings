...
### T32. 랜딩 페이지 사용 방법 상세 섹션
- 현재 `web/index.html` 은 hero + 5개 카드 그리드만. 신규 방문자가 "뭘 할 수 있고 어떻게 쓰는지" 바로 파악 어려움.
- 추가할 흐름 (섹션형, hero 아래):
  1. **포켓몬/기술/특성/도구 찾기** — 각 탭 역할 + 검색 필터 사용법 한 줄씩.
  2. **파티 빌더** — 슬롯 카드 클릭 → 포켓몬/폼/특성/도구 선택 → "상세 설정" 펼쳐 기술·SP·성격 세팅 → 분석 패널 해석.
  3. **파티 공유** — URL 복사 (슬롯 빈 경우 포함 가능) / 로컬 저장 / AI 분석.
  4. **AI 프롬프트 활용** — 6개 템플릿(약점 분석 / 교체 추천 / 기술 조합 / 남은 슬롯 채우기 / 상대 파티 카운터 / 자유 질문) 각각의 용도 1줄. "pokemon.json fetch 필요" 표시 의미 설명.
