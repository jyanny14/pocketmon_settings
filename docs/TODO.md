...
### T33. 후원 링크 — 임시 착지 완료 (2026-04-21), Sponsors 승인 대기
- **현재 상태**: footer 에 `.btn-sponsor` (커피 아이콘 + "후원하기") 삽입 완료. href 는 임시로 `https://github.com/jyanny14` (GitHub 프로필). 8 페이지 전부 반영. CSS · i18n(ko/en 라벨 + 툴팁) · `applyTranslations` 의 `data-i18n-title` 지원 전부 완성.
- Sponsors 승인 시 단순히 `git grep "github.com/jyanny14"` 로 8곳 href 교체 → 끝.
- 배치: footer 상단 출처 바로 아래. hero/header 배치는 "돈 내라" 인상이 강해 피함.
- 문구: 라벨 "후원하기" / "Sponsor" · 툴팁 "서버·도메인 비용 보전용 · 수익 목적 아님 · Sponsors 승인 대기 중이라 현재는 프로필로 연결".
- 금지어: "개발자 후원", "커피 한 잔 사주세요" 같은 개인 수익 문구 (Nintendo IP 리스크).

### T33a. 커피 후원 플랫폼 (Buy Me a Coffee / Ko-fi) — **선택지 열어둠, 현재 결정 보류**
- **사용 시점**: GitHub Sponsors 승인이 오래(수주~수개월) 지연되거나, 승인되어도 한국 유저 접근성이 나쁘다고 판단될 때 대체·병행 검토.
- **후보 플랫폼 비교**:
  | 플랫폼 | 수수료 | 상업 인상 | 장점 | 단점 |
  |---|---|---|---|---|
  | GitHub Sponsors | 0% | **최약** | "개발자 지원" 프레임 | 승인 프로세스 있음, 한국 유저 생소 |
  | Ko-fi (one-time) | 0% | 약 | UX 친숙, 수수료 0 | "크리에이터 팁" 프레임 |
  | Buy Me a Coffee | 5% | 중 | 브랜드 인지도 높음 | 수수료 5%, 서포터 공개 기본값 |
  | Patreon | 8~12% | **강** | 구독 수익 안정 | 구독 = 상업적 색채 강함, **비권장** |
  | 토스/카카오페이 | 0% | 약 | 한국 UX 최적 | 글로벌 유저 배제 |
- **권장 우선순위**: GitHub Sponsors (1순위) → Ko-fi one-time (2순위, 승인 지연 시) → BMAC (3순위) → Patreon 절대 X.
- **복수 배치 비권장**: 여러 후원 링크는 상업 인상 강화 + 사용자 선택 피로 + 각 플랫폼마다 KYC·정산 관리 부담. 기본은 **한 플랫폼만**.
- 구현 시 변경: `.btn-sponsor` href 만 교체. 아이콘·라벨은 공통.
...
- 완화: (a) 플랫폼 선택에서 "서버비/개발시간 지원" 프레임이 명확한 곳을 우선 (Sponsors > Ko-fi > BMAC), (b) 문구에서 "수익 목적 아님 · 서버비 보전" 명시, (c) 티어별 혜택(예: "$5 후원자 전용 Pokémon 분석") 절대 만들지 말 것 — 이게 명백한 상업 이용으로 해석됨, (d) **권리자 문의 시 즉시 링크 삭제 + 누적 수령액 전액 환불 절차** README 에 사전 명시.
