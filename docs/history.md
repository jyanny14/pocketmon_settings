...
- 파티 슬롯 훈련 탭 UX 개선 — 두 가지 체감 이슈 수정.
  ...
  - **SP 입력 UI 재구성**: 기존 `[label][stepper][quick]` 3-컬럼 구조로는 `−/+` 사이 `input` 이 매우 좁아 값이 잘 안 보였고, 1차로 `[«][‹][input][›][»]` 단일 행으로 바꿔도 4개 버튼이 옆에 붙어 있는 한 2자리 숫자(최대 32)가 여전히 눌렸음. 2차 수정: **입력을 별도 행으로 분리** → row 1 `[label] [   input   ]` (라벨 오른쪽 전체 너비), row 2 `[«] [‹] [›] [»]` (4등분). `.slot-card__sp-cell` 을 `grid-template-columns: auto minmax(0,1fr)` 2-col × 2-row 그리드로, 입력 font `1.05rem / 700`, 버튼은 `repeat(4, minmax(0,1fr))` 로 등폭 배치. 좁은 카드에서도 2자리 값 완전 노출. CSS `grid-template-columns: 1.4rem 1.4rem minmax(0,1fr) 1.4rem 1.4rem` 로 input 가운데 배치, `.slot-card__sp-step--jump` 모디파이어로 `«`·`»` 는 `−/+` 보다 살짝 덜 강조(border/font-weight 차). 입력값 자체는 `font-size: .95rem; font-weight: 600` 로 키워 가독성 확보. `.slot-card__sp-quick` 스타일은 삭제.
