...
- `DETAILED_RULES_{KO,EN,JA,ZH}` 에 Champions 배틀 룰 섹션 5 신규 삽입 (자기 검증을 6으로 이동) — AI 가 파티 분석·추천 시 "6마리 전원 동시 출전" 으로 잘못 가정하는 문제 차단. 포함 룰: ① **싱글 = Pick 3 / 더블 = Pick 4** 선출 포맷 명시 (모드에 따라 출전 수 분기). ② **싱글·더블 공통 메가 진화 1배틀 1회** 제한. ③ **메가 복수 후보는 유효 전략** — 실제 배틀에선 1마리만 메가 진화하지만, 파티에 메가 후보가 여러 마리 있으면 상대가 메가 타깃을 예측하기 어려워 전략적 이점 (1마리만 있으면 상대가 곧바로 특정). ④ **아이템 클로즈** — held item 은 파티 내 unique. ⑤ "선출 vs 상대" 관점으로 분석해야 함을 명시. 자기 검증 체크리스트에도 (c) **배틀 룰 패스** 추가 — 도구 중복이면 수정, 메가 복수는 체크 대상 아님. 4 언어 모두 동일 구조로 반영. 프롬프트 본문의 "싱글/더블" 모드 지정을 참조하도록 명시적으로 지시. **수정 이력**: 초안에는 "메가 1마리 채택 권장, 2마리 이상 비효율" 이라고 잘못 기술했다가 유저 지적으로 정정 — 메가 1마리는 상대에게 읽히기 쉬워 오히려 약점, 복수 후보는 정상 전략.
...
- `prompts.html` 하단 How-to 팁 · FAQ 를 첨부 기본 경로 기준으로 재작성 — 기존 문구는 여전히 "URL fetch 가 기본, 첨부는 폴백" 톤이었음. ① **Tip 1** "파티 빌더 URL 자동 입력" → "데이터 파일은 먼저 챗에 첨부하세요" (champions-data JSON 드래그앤드롭 권장을 최상단으로). ② **Tip 3** "AI가 링크 못 읽으면 파일 첨부" → "파티 빌더 먼저, 그 다음 질문 카드" (기존 tip1 의 파티 빌더 안내를 여기로 이동). ③ **Tip 2** "카드 1개 = 질문 1개" 유지. ④ **FAQ 1** 파티 URL 없이 사용 — 문구 다듬기. ⑤ **FAQ 2** "Claude·ChatGPT" → "Claude·ChatGPT·Gemini" (3사 비교 추가, 각 장점 명시). ⑥ **FAQ 5** Champions 부재 포켓몬 추천 대처 — "첨부하지 않은 상태" 가 대부분 원인이라는 점 강조. FAQ 3/4 는 유지. `web/assets/i18n.js` 4언어 × 5 키 그룹 = 40 항목 업데이트, `web/prompts.html` 한국어 fallback 텍스트도 일치시킴.

- 프롬프트 복사 본문 2차 슬림화 + 버튼 1개로 통합 — champions-data 첨부가 기본 경로가 되면서 `SHARED_DISCLAIMER`(_readme 와 중복), 폴백 JSON/HTML URL 블록, 템플릿 본문의 fetch URL 라인(`{{POKEMON_JSON_URL}}`·`{{MOVES_JSON_URL}}` 등)이 전부 중복이 됨. ① `web/assets/prompts-templates.js` — `SHARED_DISCLAIMER_{KO,EN,JA,ZH}` 4개 export 제거 + 모든 body 시작에서 `${SHARED_DISCLAIMER_XX}\n\n${STRICT_POOL_RULES_XX}` → `${STRICT_POOL_RULES_XX}` 로 단순화 (4언어 replace_all). `STRICT_POOL_RULES_{KO,EN,JA,ZH}` 를 ~220자 → ~180자 한 문단으로 추가 슬림화 (CORE RULE + `_rules` 포인터 + slug 포맷 + 재확인 지침만). 폴백 URL 블록 완전 제거. 신규 export `FOOTER_FALLBACK_{KO,EN,JA,ZH}` 추가 — 첨부 없는 유저용 rescue 한 줄. ② `web/assets/prompts.js` — `extractSlot()` 을 **식별자-only** 포맷으로 재작성: `{slug, formName, ability:slug, item:slug, moves:[slug], sps, nature}` 만 출력, gameText/effect/baseStats/forms[].abilities/moves 상세수치 전부 제거. 6마리 파티 기준 ~5-10KB → ~1.5KB (-70%). AI 는 champions-data JSON 에서 룩업. `substitute()` 에 FOOTER append + fetch URL 라인 일괄 정규식 제거(`LLMS_TXT_URL`·`POKEMON_JSON_URL` 등 9개). `renderCards()` 의 2버튼 구조(copy-url + copy-full) → 1버튼(`copy`)으로 통합. `includeData` 파라미터 제거. ③ `web/assets/i18n.js` 4언어 — `prompts.copy` 신설, `prompts.copyWithData`·`copyUrlOnly`·`urlOnlyHint`·`fetchRequired` 제거, `prompts.dataBundleStep3` 문구 업데이트. ④ 결과 — weakness 템플릿 싱글 ko 기준 본문 ~2.5KB → ~500-600자(폴백 포함), 6마리 파티 JSON 포함 총 ~1.9KB (이전 ~8KB, -75%). 매 질문 재전송되는 문맥이라 무료 GPT 토큰 한도에서 크게 유리해짐.

- 프롬프트 규칙 슬림화 + champions-data 번들에 `_rules` 내장 — 파일 첨부가 기본 경로가 된 이상, 매 프롬프트마다 ~3KB 짜리 `STRICT_POOL_RULES` 를 반복 전송하는 것은 무료 AI 토큰 낭비. 슬림 CORE RULE + 상세 `_rules` 2계층으로 재구성. ① `web/assets/prompts-templates.js` 의 `STRICT_POOL_RULES_{KO,EN,JA,ZH}` 4개를 ~600자 슬림 본으로 축약 (CORE RULE + 필수 3절차 + 첨부 없을 때 폴백). 기존 상세 조항은 신규 export `DETAILED_RULES_{KO,EN,JA,ZH}` 4개로 분리 — 부재 포켓몬/도구 예시, 폼 매칭, 불확실 대응, 자기검증 체크리스트 등. ② `web/assets/prompts.js` — `DETAILED_RULES_*` import 후 `DETAILED_RULES_BY_LANG` 맵 구성, `downloadDataBundle()` wrapper 에 `_rules: DETAILED_RULES_BY_LANG[lang]` 필드 삽입. AI 는 첨부 파일을 열면서 자연스럽게 상세 규칙을 함께 로드. ③ 슬림 RULES 본문이 "`_rules` 필드를 먼저 읽고 전부 준수하라" 고 명시적으로 지시 — 파일 내 텍스트가 "데이터"로만 취급되는 위험 완화. ④ 토큰 경제성: 프롬프트 본문 규칙 ~3KB → ~600자 (약 80% 절약). 번들 파일은 `_rules` 한 언어분 ~1.5KB 증가하지만, description 필드 정리로 상쇄 이상.
- `LANG_DROP_FIELDS.ja`/`.zh` 에 `description`/`flavorText`/`flavorTextEn`/`effect`/`gameText` 등 **영어 원문 폴백 필드 추가 드롭** — 2026-04-23 Turn B~D 에서 abilities descriptionJa/Zh, item effectJa/Zh, move flavorTextJa/Zh 가 전부 100% 커버됐으므로 영어 원문을 폴백으로 남길 이유가 사라짐. ja/zh 번들에서 descriptionKo/Ja/Zh 중 ja/zh 만, effectKo/Ja/Zh 중 해당 언어만 유지. 번들 크기 감소 (abilities description 192건 × 평균 100자 ≈ ~20KB 절약 per bundle). 기존 주석 "description 자체(영어)는 유지 — 폴백용" / "flavorTextEn 은 유지 (ja 누락 대비)" 제거.
...
- 메가 브리가론 / 메가 마폭시 / 메가 개굴닌자 입수 게임도 `legends-z-a` 전용으로 축소 — 카로스 스타터 3종 신규 메가는 Z-A 전용이라 동일 규칙 적용. 기본 폼(브리가론·마폭시·개굴닌자) 은 SV 에도 존재하므로 기존 목록 유지, 메가 폼만 `["legends-z-a"]` 로 교체. `data/manual/game_sources_override.json` `chesnaught.Mega Chesnaught` / `delphox.Mega Delphox` / `greninja.Mega Greninja` 수정 → 재빌드로 반영.

- `docs/za_exclusive_forms.md` 신규 — Z-A 전용 폼 목록을 문서화. 플라엣테 2종 + 카로스 스타터 메가 3종 포함. 추가 절차(표 갱신 → override 수정 → build.py → history 기록) 4단계 명시해서 이후 항목이 늘어날 때 체크리스트로 쓰게 함.

- 파티 분석 방어 프로필에 **특성 효과 반영** — 사용자 피드백: "속성 약점 보여줄때 포켓몬에 특성이 부유 같이 해당 데미지를 안 받는 특성이면 속성 약점에 포함 안하고 싶은데". 이전엔 `renderDefensiveProfile()` 이 `attackMultiplier(atk, form.types)` 로만 약점/저항/무효 집계해 특성 무시. 이제 `slot.abilitySlug` 를 보고 `ABILITY_DEFENSE_EFFECT` 매핑으로 배율 수정자 적용. 무효화 9종(부유·축전·저수·타오르는불꽃·초식·전기엔진·피뢰침·흙먹기·건조피부)은 ×0, 반감 4종(두꺼운지방(불꽃+얼음)·내열·수포·정화의소금)은 ×0.5 적용. 예: 부유 포켓몬은 땅 공격 약점 집계에서 제외, 두꺼운지방 포켓몬은 불꽃·얼음이 약점에서 저항으로 이동. 스톰드레인·좋은밥통·폭신폭신은 Champions 에 없거나 복합 로직이라 제외. 필터·하드락(0.75×), 건조피부 불꽃 1.25× 같은 경계-근접 효과는 분류 임계값(≥2×) 거의 영향 없어 단순화. `web/assets/party.js` 최상단에 `ABILITY_DEFENSE_EFFECT` 상수 신설 + `renderDefensiveProfile` 의 배율 계산에 `abilityMod` 적용. `i18n.js` `party.analysis.defenseHint` 를 4 언어 전부 갱신해 "특성 무효·반감 반영" 명시.

- i18n Phase 2 1단계 — ja/zh 내부 데이터(설명·효과·플레이버) PokeAPI 로 자동 수집 + 빌드 통합. 사용자 피드백: "일본어 파티빌더에서 내부 데이터들은 여전히 영어·한글 혼용인데? 내부 데이터는 언제 바꿀거야". 조사 결과 PokeAPI `flavor_text_entries` 는 ability/item/move 전부 ja/zh 수록(`effect_entries` 는 en/fr 만). (1) `scripts/fetch_flavors_i18n.py` 신규 — `--target {ability|item|move} --lang {ja|zh}` CLI, 6 조합 자동 수집. 구현 시 발견: PokeAPI `/item/` 은 flavor_text_entries 엔트리 키가 `text` 이고 `/ability/`·`/move/` 는 `flavor_text` — 두 키 모두 fallback 처리. (2) 수집 결과: ability gameText 176/192 (ja/zh 동일), item effect 93/117, move flavorText 437/481 — 나머지 ~45건은 Champions 신규 + 9세대 신규로 PokeAPI 업스트림 공백 (LLM 번역 또는 Bulbapedia 2차 수집 대상). (3) `scripts/build.py` + `scripts/build_moves.py` 확장 — `gameTextJa`/`gameTextZh` (abilities), `effectJa`/`effectZh` (items), `flavorTextJa`/`flavorTextZh` (moves) 필드 병합. 각 필드 수동 override 경로도 동시 등록 (`data/manual/{target}_{field}_{lang}.json`). (4) `web/assets/app.js` 표시 헬퍼 확장 — `abilityGameText` · `abilityDescription` · `itemEffect` 전부 4-way switch. description 필드는 ja/zh 미수집이라 영어 폴백 유지(LLM 번역 대상). (5) `web/assets/moves-list.js` 플레이버 셀 4-way + 검색 필드에 `nameJa`·`nameZh`·`flavorTextJa`·`flavorTextZh` 추가. (6) `web/assets/pokemon-detail.js` 기술 툴팁 4-way. (7) `web/assets/party.js` 기술 picker 플레이버 4-way. (8) `web/assets/prompts.js` — 인라인 JSON 언어 분기에 ja/zh 게임 텍스트 키 반영 (`gameTextJa`·`effectJa`·`flavorTextJa` 등), `LANG_DROP_FIELDS` 를 상세화해 현재 언어의 설명 필드만 남기도록 재구성. 대표 확인: `adaptability` gameText ja=`自分と おなじ タイプの 技の 威力が 上がる。` zh=`与自身同属性的招式 威力会提高。`, `focus-sash` effect ja=`持たせると ＨＰが 満タンのとき...`, `earthquake` flavor ja=`地震の 衝撃で 自分の 周りに いるものを 攻撃する。` 모두 공식 게임 내 텍스트 일치.

- i18n UI 문자열 ja/zh 전면 번역 완료 — 이전엔 핵심 네비+공통 라벨 ~35키만 번역하고 나머지 ~245키는 영어 폴백이었음. 이제 ko/en 과 **완전 키 대칭(각 282 키)** 으로 일본어/중국어 간체 전 UI 문구 번역. 고유명사 항목(타입 18종·성격 25종·HOME 연동 게임 10종·이동 분류·스탯 라벨) 은 **공식 게임 내 표기** 적용 (예: タイプ → ノーマル/ほのお/みず · 성격 → いじっぱり/がんばりや · 属性 → 一般/火/水 · 性格 → 固执/勤奋). UI 문구(버튼·필드·분석 패널·프롬프트 카드 등) 는 자연스러운 번역 + 각 언어의 조사·구두점 관행 준수(ja 는 전각 공백·점, zh 간체는 전각 점·쉼표). 파티 분석 패널, 프롬프트 페이지 설명문(인트로·데이터 번들 안내·카드 설명 등), 인덱스 페이지 How-To 4단계 등 긴 문장도 모두 반영. 검증: `node --check i18n.js` 통과, ja/zh 282 키 ko 와 완전 일치 확인. `docs/TODO.md` T-i18n-3 (UI 문자열 전면 번역) 완료 처리 예정.

- 언어 토글을 순환 버튼 → 드롭다운 리스트로 교체 — 이전 구현은 클릭할 때마다 `한→EN→日→中→한` 으로 4번 눌러야 원하는 언어에 도달해 UX 가 나빴음. 사용자 피드백: "누가 다국어 변경을 저렇게 버튼을 둬, 누르면 리스팅 되서 원하는거 선택할수 있게 바꿔". `web/assets/i18n.js` `initLangToggle()` 를 리스트박스 팝업 방식으로 재작성 — 버튼 클릭 시 `<ul role="listbox">` 를 body 에 absolute 로 부착해 4 언어(`한 한국어` · `EN English` · `日 日本語` · `中 简体中文`) 표시, 항목 클릭 시 `setLang()` 호출. 화면 오른쪽 경계 넘치면 자동 우측 정렬, 바깥 클릭·Escape 닫기, 현재 언어에 포커스, 키보드(Enter/Space) 선택 지원. HTML 8 파일 마크업은 건드리지 않음 — JS 가 초기화 시 기존 2-way span 을 단일 라벨(`LANG_LABELS[_lang]`) 로 덮어씀. `LANG_FULL_NAMES` 상수 신설로 메뉴 표시용 풀 네임 제공. `web/assets/style.css` `.lang-toggle-menu` · `.lang-toggle-menu__opt` · `.lang-toggle-menu__mark` · `.lang-toggle-menu__name` 스타일 신규(rounded corners · box-shadow · hover/active 상태).

- i18n Phase 1 ja/zh 이름 커버리지 100% 달성 — `scripts/fetch_names_from_bulbapedia.py` 신규로 PokeAPI 업스트림 공백(Champions 신규 메가스톤 + 9세대 신규 특성/기술) 을 Bulbapedia 에서 자동 보완. 총 **101건 채움** (ja 28건 + zh 73건, 실패 0). 파서 로직: (a) 첫 문단 `<b>(Name)</b> (Japanese: <b>日本語</b>` 정규식에서 일본어, (b) "In other languages" 섹션 `Mandarin</span>` 앵커 뒤 `<td>` 에서 중국어. Bulbapedia 의 `반체 / 간체` 슬래시 분리 포맷(`反芻 / 反刍`·`龍皮膚 / 龙皮肤`) 발견 후 "슬래시 뒤 토큰 = 간체" 규칙으로 파싱 수정. `scripts/config.py` `REQUEST_DELAY_BY_HOST` 에 `bulbapedia.bulbagarden.net: 5.0` 추가 (robots.txt `Crawl-delay: 5` 준수). 수집 결과는 `data/manual/*_names_{ja,zh}.json` 에 정렬 저장되며 `_verified_sources` 에 Bulbapedia 출처 + 수집일자 자동 기록. 검증 스크립트 `validate_names_i18n.py --strict` ja/zh 양쪽 exit 0 통과. 대표 샘플: `supersweetsyrup 감미로운꿀 / かんろなミツ / 甘露之蜜`, `chandelurite 샹델라나이트 / シャンデラナイト / 水晶灯火灵进化石`, `populationbomb 찍찍베기 / ネズミざん / 鼠数儿`. 이름 철칙 유지 — LLM 번역 전혀 없음, 전부 Bulbapedia 가 수록한 공식 게임 표기.

- i18n Phase 1 M2 (UI 레이어) — 데이터 레이어가 준비된 상태에서 ja/zh 토글 · 표시 헬퍼 · 인라인 JSON 언어 분기 완료. (1) `web/assets/i18n.js` — `STRINGS.ja` / `STRINGS.zh` 추가 (핵심 네비 + 공통 버튼 ~35키 LLM 초벌 번역, 나머지는 영어 폴백). `SUPPORTED_LANGS = ["ko","en","ja","zh"]` · `LANG_LABELS = {ko:"한",en:"EN",ja:"日",zh:"中"}` 상수 신설. `t()` 폴백 체인 `lang → en → ko → key` (영어를 1차 폴백으로 끌어올림). `initLangToggle()` 4-way 순환(ko→en→ja→zh→ko) 으로 재작성 — `#lang-toggle` 버튼이 현재 언어 단일 라벨(한/EN/日/中) 을 표시해 8 HTML 파일 수정 없이 동작. (2) `web/assets/app.js` — `pickNameByLang()` 헬퍼 신규로 `abilityDisplayName` / `itemDisplayName` / `moveDisplayName` / `formDisplayName` 4-way 스위치 재구성. `matchesQuery()` 검색 함수에 `nameJa` / `nameZh` 포함시켜 일·중 사용자가 자기 언어로 검색 가능. 설명/효과 필드 헬퍼는 ko 외 언어 영어 폴백(Phase 2 대기). (3) `web/assets/prompts.js` — `LANG_DROP_FIELDS` / `FORM_DROP_FIELDS` 4-way 확장. `extractSlot()` 의 `pickName` 을 4-way 로 재작성하고 gameText/effect 는 ko/en 분기로 단순화. `bundleReadme()` 에 ja (`Pokémon Champions 完全データセット...`) · zh (`Pokémon Champions 完整数据集...`) 문구 추가. `resolveBody()` 폴백 체인을 `lang → lang단순 → en{Double} → en → ko` 로 확장해 ja/zh 모드에서 en 바디 안전 폴백(Phase 3 까지 임시). 번역되지 않은 UI 상세 문자열 및 Phase 2/3 작업은 `docs/TODO.md` T-i18n-1~6 로 기록.

- i18n Phase 1 M1 (데이터 레이어) — 포켓몬·특성·도구·기술 4 종류에 일본어(`nameJa`) · 중국어 간체(`nameZh`) 필드 추가. 통합 fetch 스크립트 `scripts/fetch_names_i18n.py` 신규 (기존 `fetch_ability_names_ko.py` / `fetch_item_names_ko.py` 의 다국어 일반화판, `--target` × `--lang` CLI), `data/processed/{pokemon,ability,item,move}_names_{ja,zh}.json` 8 파일 생성. `scripts/build.py` · `scripts/build_moves.py` 확장해 ja/zh 로더 + `nameJa`/`nameZh` 필드 병합. `scripts/form_i18n.py` 신규 — `form_ko.py` 다국어 확장판으로 메가/지역폼 이름을 ja(`メガフシギバナ` 등 공백 없이 붙임) / zh(`超级妙蛙花` 등 공식 표기) 로 합성. 수동 override 8 파일(`data/manual/*_names_{ja,zh}.json`) 빈 stub 으로 생성 — **LLM 번역 금지 + 공식 출처 크로스체크 의무** 를 `_comment` 에 명시. 검증 스크립트 `scripts/validate_names_i18n.py` 신규 — Champions 신규 화이트리스트(특성 `isNewInChampions` / 아이템 `item_names_ko.json` 수동 키) 기반 누락 리포트, `--strict` 플래그로 CI 실패 유도 가능. 수집 결과: pokemon ja/zh 186/186, ability ja 187·zh 178, item ja 94·zh 93, move ja 481·zh 446 — 남은 누락은 Champions 신규 메가스톤·9세대 신규 특성/기술에 대한 PokeAPI 업스트림 공백(`docs/TODO.md` T-i18n-1~2 기록). 슬러그 이슈 하나 발견·수정: `mr.rime` 은 PokeAPI `/pokemon-species/mr-rime/` 이므로 `POKEMON_SLUG_OVERRIDES` 에 매핑 추가. 대표 확인(venusaur `이상해꽃/フシギバナ/妙蛙花`, 메가 `메가 이상해꽃/メガフシギバナ/超级妙蛙花`, earthquake `지진/じしん/地震`) 모두 공식 표기로 통과.

- i18n Phase 1 Step 1 (PokeAPI 언어 코드 실측) — `curl /api/v2/{pokemon-species,ability,item,move}/1/` 로 `names[].language.name` 확인. 4 엔드포인트 모두 동일한 언어셋 제공: `de, en, es, fr, it, ja, ja-hrkt, ko, zh-hans, zh-hant`. 포켓몬 이름은 `ja` / `ja-hrkt` 값이 같아(フシギダネ) `ja` 가 더 표준이라 채택. 간체는 `zh-hans` 확정, 번체(`zh-hant`) 는 Phase 1 범위 밖이라 비채택. `scripts/config.py` 에 `POKEAPI_LANG_CODES = {"ko":"ko","ja":"ja","zh":"zh-hans"}` 매핑 상수 추가. 이 상수는 후속 `fetch_names_i18n.py` / `fetch_flavors_i18n.py` 에서 공용.

- `docs/plan_i18n.md` 에 "핵심 원칙" 섹션 추가 — 포켓몬·도구·기술·특성 **이름 필드는 PokeAPI 공식 지역화 이름만** 사용 (LLM 번역·사람 임의 번역·음차 전면 금지). 사유: 유저가 자기 게임 화면에서 본 이름으로 검색할 때 매칭이 돼야 하므로, 공식 표기와 한 글자라도 어긋나면 로컬라이징 실패. 수동 override 는 PokeAPI 에 없는 항목(Champions 신규 메가스톤·신규 특성 등) 에만 허용하며, 공식 출처(공식 사이트 일/중 판, Bulbapedia ja/zh, 공식 트레일러) 크로스체크 + `_comment` 에 검증 출처 기록 의무화. 설명·효과 필드는 Phase 2 에서 별도 규칙(LLM 번역 허용, 단 플래그 + 검수). 검증 스크립트 `scripts/validate_names_i18n.py` 를 Phase 1 체크리스트에 추가 — 누락 항목이 Champions 신규 외에 있으면 빌드 실패로 처리.

- `docs/plan_i18n.md` 신규 — 일본어 / 중국어(간체) 지원 3-Phase 계획. Phase 1 (이름·UI) / Phase 2 (설명·효과) / Phase 3 (AI 프롬프트·HTML 레퍼런스) 로 분리해 각 단계를 독립 배포 가능하게 설계. 필드 확장은 기존 `nameKo`+`nameEn` 접미사 패턴 유지(`nameJa`·`nameZh` 추가) — 객체 구조 전환은 20+ 표시 헬퍼 재작성 부담 때문에 비채택. PokeAPI 언어 코드 실측(`ja` vs `ja-Hrkt`, `zh-Hans` vs `zh`) 을 1단계 선행 작업으로 명시. 언어 토글 UI 는 2-way 버튼 → `<select>` 드롭다운 권장(4 언어 확장성). 프롬프트 템플릿 39종 × 4 언어 × 2 모드 = 312 바디 폭증 위험은 `SHARED_DISCLAIMER` / `STRICT_POOL_RULES` 를 언어별 상수로 분리하고 질문 본문만 번역하는 식으로 완화. 레퍼런스 HTML 은 ko+en 병기 단일 파일 → 언어별 파일 분리로 재설계. 폰트 폴백 (`Noto Sans JP/SC`), `<html lang>` 동적 갱신, 번역 품질 검수 워크플로, 예상 일정 포함.

- `README.md` "데이터 출처 / 라이선스" 섹션 개편 — 기존엔 `## 데이터 출처` (bullet 3줄) 과 `## 라이선스 / 면책` (bullet 4줄) 이 분리돼 있어 **어떤 출처가 어떤 라이선스** 로 쓰이는지 매핑이 약했음. 두 섹션을 **`## 데이터 출처 / 라이선스`** 하나로 병합하고 (1) Pokémon 권리 블록 (Nintendo/Creatures/GAME FREAK · 비영리 · 요청 시 삭제) + (2) **3-열 표** (출처 / 용도 / 라이선스 — serebii · PokeAPI(BSD-3) · PokeAPI/sprites(MIT)) 구조로 재작성. 표에서 serebii 가 "도구 아이콘 PNG" 용도도 함께 담당함을 명시. 말미에 "serebii 의존을 줄이려 하지만 Champions 고유 데이터 (`updatedInChampions`/`isNewInChampions` 등) 가 PokeAPI 에 없어 현재는 대체 불가" 사유 1문단 추가. 섹션 위치는 `기술 스택` **뒤** 로 이동 (독자가 "무얼 만드는지 → 기술 스택 → 출처" 순으로 읽도록). 배경: 사용자 대화에서 스프라이트 출처가 이미 PokeAPI 이지 serebii 가 아니라는 점이 확인됐고, 명시적 라이선스 표기(MIT/BSD-3) 가 기존엔 "팬 프로젝트 관행" 수준으로 약했음.

- AI 프롬프트 규칙 1 을 Step 0 / Step 1 두 블록으로 재구성 — 사용자 피드백: 이미 첨부 파일을 올린 뒤에도 AI(특히 Gemini) 가 URL fetch 를 다시 시도하거나 재-첨부 요청을 반복하는 문제. 기존 규칙 1 은 `(a-0) 첨부 > (a) fetched JSON > ...` 순서를 **한 문단 안에** 담고 있어 첨부 우선순위가 묻혀 있었음. 이제 **Step 0 (첨부 단락 · short-circuit — 발견 시 URL fetch·재첨부 요청·이전 턴 요약 재사용 모두 금지, 첨부 파일만 직접 재조회)** / **Step 1 (첨부 없을 때만 기존 폴백 체인)** 으로 분리. 규칙 2 도 "Step 1 의 (a)(b)(c) 가 모두 불가능한 경우" 로 문두 수정해 연결 명확히. 규칙 3~7 (slug 병기·부재 예시·자기 검증 등 환각 방지) 은 그대로 유지. `web/assets/prompts-templates.js` `STRICT_POOL_RULES_KO` / `STRICT_POOL_RULES_EN` 양쪽 대칭 적용 — 39 템플릿은 모듈 로드 시 상수를 baked-in 하므로 자동 반영. `docs/ai-prompts.md` 에 "Step 0 / Step 1 구조" 한 단락 동기화. 배경: fetch 결과는 클라이언트 요약으로 손실될 수 있는 반면 첨부 파일은 ChatGPT/Claude.ai 에서 재조회 가능한 리소스로 남는다는 전제.
