...
- 파티 슬롯 훈련 탭 UX 개선 — 두 가지 체감 이슈 수정.
  ...
  - **SP 입력 UI 확장**: 기존에 SP 셀이 `[label][stepper][quick]` 3-컬럼 구조라 `−/+` 사이 `input` 이 매우 좁아서 현재 값이 안 보였음. 또 `MAX`·`0` 이 별도 2버튼 행을 차지해 스크롤 부담도 큼. 레이아웃을 한 줄로 통합: `[«] [‹] [input] [›] [»]` — `‹ / ›` 는 1씩 감·증, `« / »` 는 0 으로 초기화 / 예산 한도까지(66점 부담·32점 상한 반영) 즉시 채움. 네 버튼이 같은 화살표 계열이라 시각적 일관성 확보. CSS `grid-template-columns: 1.4rem 1.4rem minmax(0,1fr) 1.4rem 1.4rem` 로 input 가운데 배치, `.slot-card__sp-step--jump` 모디파이어로 `«`·`»` 는 `−/+` 보다 살짝 덜 강조(border/font-weight 차). 입력값 자체는 `font-size: .95rem; font-weight: 600` 로 키워 가독성 확보. `.slot-card__sp-quick` 스타일은 삭제.
