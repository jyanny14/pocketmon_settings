...
def write_manifest(
    ...
    # natures.json is generated by build_natures.py — include if present.
    natures_path = DIST_DIR / "natures.json"
    if natures_path.exists():
        try:
            natures_data = json.loads(natures_path.read_text(encoding="utf-8"))
            natures_count = len(natures_data) if isinstance(natures_data, list) else 0
        except Exception:  # noqa: BLE001
            natures_count = 0
        files.append(_file_entry("natures", natures_count))
