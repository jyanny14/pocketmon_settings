 ...
 * 제외 항목 — 이유별 구분:
 *
 * 1) **Champions 라인업 외 특성** (abilities.json 에 아예 없음):
 *    - `stormdrain` (노심융해, 물 무효) — Gastrodon 등 보유 포켓몬이 Champions 에 없음.
 *    - `wellbakedbody` (노릇노릇바디, 불꽃 무효) — Dachsbun / Fidough 가 Champions 에 없음.
 *    특성 수집 파이프라인(`scripts/fetch_abilities.py`) 은 Champions 포켓몬 상세 페이지에
 *    링크된 slug 만 모으므로, 보유 포켓몬이 라인업에 없으면 특성 자체가 자연 누락된다.
 *    향후 해당 포켓몬이 추가되면 재-fetch 만으로 자동 편입됨.
 *
 * 2) **로직이 복잡해 분류 임계값 대비 효용 낮음** (있지만 의도적으로 생략):
 *    - `filter` / `solidrock` / `prismarmor` (효과 뛰어남 0.75×) — 기존 mul 이 2 이상일 때만
 *      의미 있고 1.5 로 내려도 여전히 weak 분류(≥2) 경계 근처라 단순 약점 토글이 안 됨.
 *    - 건조피부의 불꽃 1.25× 증가(건조피부가 물 무효는 위에 반영됨) — 분류 임계값
 *      ≥2× 를 넘기려면 이미 타입 상성에서 2× 이상이어야 해 실제 집계 변화 거의 없음.
 *    - `fluffy` (폭신폭신) — 불꽃 2× + 접촉 0.5× 복합. 접촉 여부는 기술 단위 정보라
 *      파티 전체 방어 프로필 수준에서 단순화 어려움.
 *
 * 3) **타입 기반 데미지 경감이 아닌 특성** (해당 없음으로 제외):
 *    - `magicguard` · `overcoat` · `wonderguard` 등 — 간접 피해/가루/상성 무시 같은
 *      조건부 무효라 본 맵 범위와 무관.
 */
